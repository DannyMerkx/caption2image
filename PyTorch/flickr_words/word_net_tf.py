...
from encoders import img_encoder, text_rnn_encoder, text_transformer
...
parser.add_argument('-results_loc', type = str, default = '/data/caption2image/PyTorch/flickr_words/results/',
                    help = 'location to save the results and network parameters')
parser.add_argument('-dict_loc', type = str, default = '/data/caption2image/PyTorch/flickr_words/flickr_indices')
...
token_config = {'embed':{'num_chars': dict_size, 'embedding_dim': 1024, 'sparse': False, 'padding_idx': 0}, 
               'tf':{'input_size': 1024, 'fc_size': 2048, 'n_layers': 6, 'h': 8, 'max_len': 64}, 'cuda': cuda}
...
cap_net = text_transformer(token_config)
...
cyclic_scheduler = create_cyclic_scheduler(max_lr = args.lr, min_lr = args.lr * 0.2, stepsize = (int(len(train)/args.batch_size)*5)*4)
